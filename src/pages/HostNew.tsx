  ...
  return (
    <PageShell>
      <div className="container-tight max-w-3xl">
        <span className="editorial-eyebrow text-coral">Become a host</span>
        ...
        <ol className="flex items-center gap-2 mt-10 mb-8 overflow-x-auto pb-2">
          {STEPS.map((s, i) => (
            <li key={s} className="flex items-center gap-2 text-sm whitespace-nowrap">
              <span className={cn("h-7 w-7 inline-flex items-center justify-center rounded-full text-xs font-bold border border-white/10",
                i < step ? "bg-cobalt text-white" : i === step ? "bg-gradient-sunset text-white shadow-glow-coral glossy" : "glass text-muted-foreground")}>
                {i < step ? <Check size={14} /> : i + 1}
              </span>
              <span className={cn(i === step ? "font-semibold text-foreground" : "text-muted-foreground")}>{s}</span>
              {i < STEPS.length - 1 && <span className="w-6 h-px bg-white/10" />}
            </li>
          ))}
        </ol>

        <div className="rounded-3xl widget p-7">
          {step === 0 && (
          ...
          {step === 1 && (
            <div>
              <div className="rounded-3xl border-2 border-dashed border-white/15 p-12 text-center bg-white/5">
                ...
                <Button variant="glass" className="mt-4">Browse files</Button>
              </div>
            ...
            <div className="space-y-5">
              ...
              <Field label="Amenities">
                <div className="flex flex-wrap gap-2">
                  {AMENITIES.map((a) => (
                    <button key={a} type="button" onClick={() => toggle("amenities", a)} className={cn("px-3 py-1.5 rounded-full text-xs font-medium border transition-all press",
                      data.amenities.includes(a) ? "bg-gradient-cobalt text-white border-white/15 glossy" : "glass border-white/10 hover:bg-white/15")}>{a}</button>
                  ))}
                </div>
              </Field>
              <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="House rules (one per line)"><textarea rows={4} className={cn(inp, "py-3 h-auto")} value={data.rules} onChange={(e) => update("rules", e.target.value)} /></Field>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              <div className="rounded-2xl bg-gradient-cobalt/30 border border-cobalt/20 p-4 text-sm">
              ...
              <Field label="Available days">
                <div className="flex flex-wrap gap-2">
                  {["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((d) => (
                    <button key={d} type="button" onClick={() => toggle("days", d)} className={cn("h-12 w-12 rounded-2xl text-sm font-bold border transition-all press",
                      data.days.includes(d) ? "bg-gradient-sunset text-white border-white/15 shadow-glow-coral glossy" : "glass border-white/10 hover:bg-white/15")}>{d}</button>
                  ))}
                </div>
              </Field>
              <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="flex justify-between mt-6">
          <Button variant="glass" onClick={prev} disabled={step === 0}><ChevronLeft size={16} /> Back</Button>
          {step < STEPS.length - 1
