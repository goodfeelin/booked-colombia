    ...
    <PageShell>
      <div className="container-tight max-w-md">
        <div className="text-center">
          <span className="editorial-eyebrow">Booked Colombia</span>
        ...
        {mode === "signup" && (
          <div className="mt-6 grid grid-cols-2 gap-2 p-1 bg-secondary rounded-full">
            {[
              { k: "guest", label: "I'm a creator", icon: Camera },
              { k: "host", label: "I'm a host", icon: Sparkles },
            ].map((r) => (
              <button key={r.k} onClick={() => setRole(r.k as any)} className={cn("py-2.5 rounded-full text-sm font-semibold inline-flex items-center justify-center gap-2 transition-all",
                role === r.k ? "bg-card shadow-soft" : "text-muted-foreground")}>
        ...
        <form onSubmit={submit} className="mt-6 rounded-3xl bg-card border border-border shadow-card p-6 space-y-4">
          {mode === "signup" && (
            <Field icon={Sparkles} placeholder="Full name" type="text" required />
          )}
          ...
          <Button type="button" variant="outline" className="w-full">Continue with Apple</Button>
        </form>
...
const Field = ({ icon: Icon, ...props }: { icon: any } & React.InputHTMLAttributes<HTMLInputElement>) => (
  <label className="flex items-center gap-3 h-12 px-4 rounded-2xl border border-border bg-background focus-within:border-foreground transition-colors">
